...
std::vector<std::size_t> find_path_AStar(Graph& graph)
{
    ...
    min_weigth_map.insert({ 0, graph.start_index });
    while (!min_weigth_map.empty())
    {
        auto [current_key, current_index] = *(min_weigth_map.begin());
        ...
        for (std::size_t i = 0; i < graph.nodes[current_index].edges.size(); i++)
        {
            ...
            if (!graph.nodes[index_to].visited)
            {
                if (current_weight + edged_weight < graph.nodes[index_to].weight)
                {
                    ...
                    min_weigth_map.insert({ euristic_weight(graph.nodes[index_to], graph.nodes[graph.end_index]), index_to });
                }
            }
        }
